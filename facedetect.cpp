...
const char* nested_cascade_name =
    "../opencv-1.1.0/data/haarcascades/haarcascade_eye_tree_eyeglasses.xml";
...
//    }
//    else
//        image = cvLoadImage( "testimage.png", 1 );
//
...
//    }
	...
	detect_and_draw( image );
	cvSaveImage("facedetected.png",image);
	cvWaitKey(0);
	cvReleaseImage( &image );
...
void detect_and_draw( IplImage* img )
{
    ...
    if( cascade )
    {
        ...
        for( i = 0; i < (faces ? faces->total : 0); i++ )
        {
			...
			std::cout<<r->x<<","<<r->y<<" "<<r->width<<","<<r->height<<" "<<radius<<std::endl;
            if( !nested_cascade )
                continue;
